-- Bre/Core/UI.lua
-- Main UI shell (WA-like) for Bre. No engine logic; reads BrelmsSaved.displays only.
...
function UI:EnsureFrame()
  ...
  local f = CreateFrame("Frame", "BrelmsMainFrame", UIParent, "BackdropTemplate")
...
-- Tree scroll
	local scroll = CreateFrame("ScrollFrame", "BrelmsTreeScroll", left, "UIPanelScrollFrameTemplate")
	...
	-- Search EditBox remains interactable above the Tree.
	  ...
	  local sb = scroll.ScrollBar or _G["BrelmsTreeScrollScrollBar"]
...
scroll:SetScript("OnMouseWheel", function(selfFrame, delta)
  local sb = selfFrame.ScrollBar or _G["BrelmsTreeScrollScrollBar"]
...
left._ClampTreeScroll = function()
  ...
  local hdrLoaded = content:CreateFontString(nil, "OVERLAY", "GameFontNormal")
  ...
  local hdrUnloaded = content:CreateFontString(nil, "OVERLAY", "GameFontNormal")
  ...
  f._expanded = {}
  if BrelmsSaved and BrelmsSaved.ui and BrelmsSaved.ui.tree and type(BrelmsSaved.ui.tree.expanded) == "table" then
    for k, v in pairs(BrelmsSaved.ui.tree.expanded) do
...
function UI:BuildNewOverlay(parent)
  ...
  local rightTitle = rightBlock:CreateFontString(nil, "OVERLAY", "GameFontNormal")
...
function UI:ShowTreeRowContextMenu(nodeId, anchorFrame)
  ...
  if not f._rowMenuFrame then
    f._rowMenuFrame = CreateFrame("Frame", "Brelms_RowContextMenu", UIParent, "UIDropDownMenuTemplate")
  end
...
function UI:OpenExportWindow(text)
  ...
  local w = CreateFrame("Frame", "BrelmsExportWindow", UIParent, "BackdropTemplate")
...
row:HookScript("OnLeave", function(self)
  ...
  if HAS_EXPAND_SLOT then
    ...
    if hasChildren then
      ...
      local t = ex:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
      t:SetPoint("CENTER")
  ...
  local mark = cb:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
...
-- Group icon (inherits)
  ...
  if showMove then
    local ARROW_TEX = "Interface\\AddOns\\Bre\\Media\\Textures\\arrow.tga"
...
function UI:_RebuildIndexAndRefresh()
  ...
  if Move and Move.RefreshAutoLevelsByTree then
    pcall(function() Move:RefreshAutoLevelsByTree("BACKGROUND") end)
  end
end
...
function UI:_EnsureRootOrder()
  -- DB initialization is centralized in Core (Gate→DB)
  return (BrelmsSaved and BrelmsSaved.rootChildren) or {}
end
...
function UI:BeginTreeDrag(id)
  ...
  if not f._dragTip then
    ...
    local fs = tip:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    fs:SetPoint("CENTER")
...
if eg and eg.RunGuarded then
  return eg:RunGuarded("RefreshRight", function()
    ...
    --   ep:OpenDrawer("CustomMat")  -> right pane uses Brelms_ElementPaneScroll (see BuildElementPane).
...
function UI:OpenImportWindow()
  ...
  local w = CreateFrame("Frame", "BrelmsImportWindow", UIParent, "BackdropTemplate")
  -- Raise strata/level so it won't be covered by the main panel.
  w:SetFrameStrata("DIALOG")
  w:SetToplevel(true)
  if self.frame and w.SetFrameLevel then w:SetFrameLevel((self.frame:GetFrameLevel() or 0) + 50) end
  ...
  local hint = w:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
...
-- - ScrollFrame:   Brelms_ElementPaneScroll  (scroll variable: `scroll`)
-- - ScrollBar:     scroll.ScrollBar or _G["Brelms_ElementPaneScrollScrollBar"]
...
local scroll = CreateFrame("ScrollFrame", "Brelms_ElementPaneScroll", customDrawer, "UIPanelScrollFrameTemplate")
...
function _SampleElemPaneScrollMetrics()
  ...
  local sb = sf.ScrollBar or _G["Brelms_ElementPaneScrollScrollBar"]
...
scroll:SetScript("OnMouseWheel", function(self, delta)
  local sb = self.ScrollBar or _G["Brelms_ElementPaneScrollScrollBar"]
  if not sb then return end
...
do
 ...
 -- Step2: allow one-row smoke test content to render safely
  ...
  local t = pmContent:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
  ...
  local function _MakeLabel(textKeyOrText, x, y)
    local fs = pmContent:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    fs:SetPoint("TOPLEFT", x, y)
    fs:SetTextColor(YELLOW_R, YELLOW_G, YELLOW_B)
...
local lp = content
  ...
  local attrTitle = lp:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
  ...
  alphaNum:SetFontObject(ChatFontNormal)
  ...
  rotNum:SetFontObject(ChatFontNormal)
  ...
  hNum:SetFontObject(ChatFontNormal)
  ...
  wNum:SetFontObject(ChatFontNormal)
  ...
  local posTitle = lp:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
...
MakeLabel(lp, L("ELEM_MAT_FRAME_LEVEL"), 210, (-450 + POS_DY))
  ...
  xNum:SetFontObject(ChatFontNormal)
  ...
  yNum:SetFontObject(ChatFontNormal)
