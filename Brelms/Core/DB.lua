...
function DB:InitSaved()
  BrelmsSaved = BrelmsSaved or {}
  BrelmsSaved.meta = BrelmsSaved.meta or {}
  BrelmsSaved.meta.version = Bre.Const and Bre.Const.VERSION or (BrelmsSaved.meta.version or "v2.7.x")
  BrelmsSaved.meta.schema = BrelmsSaved.meta.schema or 1
  ...
  BrelmsSaved.ui = BrelmsSaved.ui or {}
  BrelmsSaved.ui.tree = BrelmsSaved.ui.tree or {}
  if type(BrelmsSaved.ui.tree.width) ~= "number" then
    BrelmsSaved.ui.tree.width = 260
  end
  BrelmsSaved.ui.tree.expanded = BrelmsSaved.ui.tree.expanded or {}

  -- UI selection state (optional; persisted)
  if type(BrelmsSaved.ui.selectedId) ~= "string" then
    BrelmsSaved.ui.selectedId = nil
  end
...
function DB:GetDisplays()
  if not BrelmsSaved or not BrelmsSaved.displays then
    return nil
  end
...
function DB:SeedMockIfEmpty()
  ...
  local function ensureSystemProgressMat()
    ...
    BrelmsSaved.rootChildren = BrelmsSaved.rootChildren or {}
    pushUnique(BrelmsSaved.rootChildren, id)
  end
  ...
  if BrelmsSaved.meta._mockSeeded then
    return
  end
  ...
  BrelmsSaved.meta._mockSeeded = true
end
