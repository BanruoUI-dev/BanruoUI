...
local function _IterAllNodeIds()
  -- Prefer DB-backed TreeIndex when available; fall back to BreSaved.displays.
  local T = _TreeIndex()
  if T and type(T.Build) == "function" then
    ...
    return coroutine.wrap(function()
      ...
      if BreSaved and type(BreSaved.displays) == "table" then
        for id, _ in pairs(BreSaved.displays) do
...
function TargetService:_EnsurePickOverlay()
  ...
  local ov = CreateFrame("Button", "Bre_PickOverlay", UIParent, "BackdropTemplate")
