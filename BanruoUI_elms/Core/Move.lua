...
function M:EnsureRuntimeRoot()
  ...
  local root = CreateFrame("Frame", "BreFrame", UIParent)
...
function M:EnsureRegion(id)
  ...
  local f = CreateFrame("Frame", "BreRegion_"..safeId, root)
  f:SetSize(64, 64)
...
function M:RefreshAutoLevelsByTree(targetStrata, idx)
  -- Auto-reorder FrameLevel for siblings within the same FrameStrata, based on Tree order (top->bottom).
  -- Notes:
  -- - Does NOT write DB. Pure runtime layer sync.
  -- - Works for all strata; if targetStrata is provided, only that strata is processed.
  if not GetData then return end
  idx = idx or _buildIndex()
  local map = idx.childrenMap or {}
  ...
  local function _actualStrata(cid, el, f)
    local props = type(el.props) == "table" and el.props or {}
    local fs = props.frameStrata
    if fs == nil or fs == "" or fs == "AUTO" or fs == "自动" then
      if f and f.GetFrameStrata then
  ...
  local function _applyForStrata(parentId, kids, strata)
    local elig = {}
    for i = 1, #kids do
      local cid = kids[i]
      if type(cid) == "string" and cid ~= "" then
        ...
        if f and f.SetFrameLevel and f.GetFrameLevel then
          ...
          if type(el) == "table" then
            local fs = _actualStrata(cid, el, f)
            if fs == strata then
    ...
    for i = 1, n do
      ...
      local f = self._regions and self._regions[cid] or nil
      if f and f.SetFrameLevel and f.GetFrameLevel then
        local p = f.GetParent and f:GetParent() or nil
        local pLvl = (p and p.GetFrameLevel and p:GetFrameLevel()) or 0
        -- Ensure stable increasing levels so earlier Tree items (top) are above later ones.
        local lvl = pLvl + 20 + i
  ...
  for parentId, kids in pairs(map) do
    if type(kids) == "table" and #kids > 0 then
      if type(targetStrata) == "string" and targetStrata ~= "" then
        _applyForStrata(parentId, kids, targetStrata)
      else
        for _, strata in ipairs(STRATA_ORDER) do
          _applyForStrata(parentId, kids, strata)
        end
      end
    end
  end
...
function M:RestoreAll()
  ...
  if not BreSaved or type(BreSaved.displays) ~= "table" then return end
  for id, el in pairs(BreSaved.displays) do
...
function M:EnsureMoverFrame()
  ...
  local f = CreateFrame("Frame", "BreMoverBody", UIParent, "BackdropTemplate")
  f:SetSize(80, 80)
...
local function _styleEditBox(eb)
  ...
  eb:SetFontObject(Bre.Font and Bre.Font:Small() or "GameFontNormalSmall")
...
function M:EnsureGroupBox()
  ...
  local f = CreateFrame("Frame", "BreGroupBox", UIParent, "BackdropTemplate")
  f:SetSize(40, 40)
  f:SetFrameStrata("DIALOG")
...
local function _ensureRootChildren()
  BreSaved = BreSaved or { displays = {}, rootChildren = {} }
