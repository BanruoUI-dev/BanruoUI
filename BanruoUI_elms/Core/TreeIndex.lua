...
-- Build parent/children index from BreSaved.displays. v2.7.12
...
function T:Build()
  ...
  -- Root order: prefer BreSaved.rootChildren when present.
  ...
  if BreSaved and type(BreSaved.rootChildren) == "table" then
    for _, rid in ipairs(BreSaved.rootChildren) do
      if rootSet[rid] then
